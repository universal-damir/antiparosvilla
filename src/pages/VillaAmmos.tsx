...
const VillaAmmos: React.FC = () => {
  const [currentImageIndexes, setCurrentImageIndexes] = useState<{ [key: string]: number }>({});
  const [galleryOpen, setGalleryOpen] = useState(false);
  const [activeGallery, setActiveGallery] = useState<{ id: string; images: string[] } | null>(null);
  const [visibleCards, setVisibleCards] = useState<Set<string>>(new Set());

  const villa = villaData.find(v => v.id === 'villa-ammos')!;
  const rooms = roomData.filter(room => room.villaId === 'villa-ammos');
